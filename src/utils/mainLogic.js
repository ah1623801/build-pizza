export async function initApp() {
  ...
  // 1. تحميل GSAP و ScrollTrigger ذاتياً لو تأخر المتصفح
  if (!window.gsap) {
    console.log('🍕 [FORNO 2/4] Fetching GSAP dynamically...');
  ...
  if (!gsap) {
    console.error('❌ [FORNO] GSAP could not be loaded.');
    return;
  }
  ...
  console.log('🍕 [FORNO 3/4] Animation engine ready!');

  /* ================= MOBILE SIZE INITIAL STAGE ================= */
  let mobileSizeChosen = false;

  function triggerSizeAlert() {
  ...
  function setupMobileSizeAnimation() {
    ...
    if (typeof state !== 'undefined') state.size = null;
...
if (builder) {
      builder.classList.add('picking-size');
    }
...
const isLandscape = window.innerWidth > window.innerHeight;
    ...
    if (window.innerWidth > 1400 && stageHost && builder) {
      ...
      const hostCenterX = (hRect.left - bRect.left) + hRect.width / 2;
      const hostCenterY = (hRect.top - bRect.top) + hRect.height / 2;
      const stageShadow = document.querySelector('.stage-shadow');

      const deltaX = (builder.offsetWidth / 2) - hostCenterX;
      const deltaY = (builder.offsetHeight / 2) - hostCenterY + 140;

      gsap.set(stageHost, { x: deltaX, y: deltaY, scale: 1.06 });
      if (stageLight) gsap.set(stageLight, { x: deltaX, y: deltaY, scale: 1.25 });
      if (stageShadow) gsap.set(stageShadow, { autoAlpha: 0 });
    }
...
document.querySelectorAll('.ms-circle-btn').forEach(btn => {
      btn.onclick = (e) => {
        e.stopPropagation();
...
function sauceSVG(type){
    ...
    let defs='<defs><radialGradient id="'+gid+'" cx="45%" cy="42%" r="68%"><stop offset="0%" stop-color="'+conf.c1+'"/><stop offset="68%" stop-color="'+conf.c2+'"/><stop offset="100%" stop-color="'+conf.c3+'"/></radialGradient><radialGradient id="'+gl+'"><stop offset="0%" stop-color="rgba(255,255,255,.32)"/><stop offset="100%" stop-color="rgba(255,255,255,0)"/></radialGradient>';
    if(!mob) defs += '<filter id="'+fid+'" x="-12%" y="-12%" width="124%" height="124%"><feTurbulence type="fractalNoise" baseFrequency="0.05" numOctaves="3" seed="'+((Math.random()*100)|0)+'" result="n"/><feDisplacementMap in="SourceGraphic" in2="n" scale="7"/></filter>';
    defs += '</defs>';

    let body = defs + '<g '+(mob?'':'filter="url(#'+fid+')"')+'>'
  ...
  function meltSVG(){
    ...
    let defs = '<defs>'
      ...
      +'<radialGradient id="'+spot+'"><stop offset="0%" stop-color="#8a3d12"/><stop offset="70%" stop-color="rgba(138,61,18,.5)"/><stop offset="100%" stop-color="rgba(138,61,18,0)"/></radialGradient>'
    ...
    if(!mob){
      defs += '<filter id="'+fid+'" x="-10%" y="-10%" width="120%" height="120%"><feTurbulence type="fractalNoise" baseFrequency="0.04" numOctaves="3" seed="'+((Math.random()*100)|0)+'" result="n"/><feDisplacementMap in="SourceGraphic" in2="n" scale="8"/></filter>';
    }
    defs += '</defs>';

    return mkSVG(200, defs + '<g ' + (mob ? '' : 'filter="url(#'+fid+')"') + '>' + '<path d="'+wobbleCircle(100,100,68,18,.06)+'" fill="url(#'+gid+')"/>' + peeks + spots + '</g>' + gloss);
  }
 ...
 const SAUCE = [
  ...
  class PizzaStage{
    constructor(host,opts){
      ...
      // دوران ناعم فقط على الديسكتوب لتوفير معالج الموبايل
...
const freshState = () => ({
    size: null,         // ⬅️ الحجم فارغ ليجبر العميل على اختياره عند كل ريفريش
    dough: 'classic',
    sauce: null,
    cheese: null,
    meats: {},
  ...
  // دالة مساعدة لقراءة سعر المكون حسب الحجم المختار
  function getIngPrice(item, size) {
    if (!item) return 0;
    const sz = size || state.size || 'med';
...
function unitPrice(s) {
    let p = 0;
    const sz = s.size || state.size || 'med';
...
function priceOf(arr, id, size) {
  ...
  function effectiveUnit() {
    return combo ? combo.basePrice + comboDelta() : unitPrice(state);
  }
...
function goStep(i){
  ...
  function renderPanel(){
    ...
    // جلب البيانات مع دعم خطوة المقاس
    ...
    body.querySelectorAll('.opt').forEach(btn => {
      btn.addEventListener('click', e => {
        ...
        if (s.single) {
          ...
          // عند اختيار المقاس: نكتفي بحفظ المقاس وتحديث السعر بدون لمس حجم البيتزا على الإطلاق
          if (s.id === 'size') {
            state.size = id;
          }
...
function renderSummary(){
    ...
    // إظهار الحجم المختار كأول سطر في كارت الفاتورة
    ...
    for(const[id,q] of Object.entries(state.meats)){
      ...
      rows.push({st:4, t:m.name+' × '+q.toUpperCase(), p: meatPrice(id,q,state.size)});
    }
    for(const v of state.vegs){
      const m=VEG.find(x=>x.id===v);
      rows.push({st:5, t:m.name, p: getIngPrice(m, state.size)});
    }
    for(const e of state.extras){
      const m=EXTRAS.find(x=>x.id===e);
      rows.push({st:6, t:m.name, p: getIngPrice(m, state.size)});
    }
...
$('#btnCartAdd')?.addEventListener('click', () => {
    ...
    // نغلق الفرن ونفتح السلة مع الإبقاء على نفس البيتزا باختياراتها في الشاشة
    closeOven(() => { 
      openCart(); 
    });
    toast('ADDED TO CART 🍕');
...
/* ================= CART ================= */
  ...
  function popBadge(){
    ...
    if(b) {
      b.textContent = total;
      gsap.fromTo(b,{scale:1.6},{scale:1,duration:.5,ease:'elastic.out(1,.4)'});
    }
    if(mb) {
      mb.textContent = total;
      gsap.fromTo(mb,{scale:1.4},{scale:1,duration:.4,ease:'power2.out'});
    }
  }
...
$('#mobileHeaderCart')?.addEventListener('click', openCart);
  ...
  function closeCart(){
    ...
    // 🛑 إيقاف أي طلبات شغالة في الخلفية فور إغلاق السلة
...
// تفعيل النسخ الآمن مع دعم المتصفحات الداخلية
...
if (customerPollInterval) clearInterval(customerPollInterval);
    customerPollInterval = setInterval(async () => {
      try {
        ...
        // 🛑 إذا لم يجد الطلب (404) أو حدث خطأ، أوقف التكرار فوراً
        if (!res.ok) {
          clearInterval(customerPollInterval);
          customerPollInterval = null;
        ...
        // 🛑 إذا اكتمل الطلب أو تم رفضه، أوقف التكرار فوراً
...
if (ord.payment_status === 'rejected') {
      ...
      // 🛑 إيقاف الريكوستات فوراً ومسح الطلب المرفوض من الكاش
      if (customerPollInterval) clearInterval(customerPollInterval);
      try { localStorage.removeItem('forno_active_order'); } catch (e) {}
      return;
    }
  ...
  function summarize(s){
    ...
    if (szObj) parts.push(szObj.name); // يكتب الحجم في أول الفاتورة
...
const ingRes = await fetch('/api/ingredients');
  ...
  function renderMenu(){
    ...
    track.innerHTML = items.map((it, n) =>
     ...
     '<button class="mbtn">ADD TO CART</button></div></article>'
    ).join('');
...
card.querySelector('.mbtn').addEventListener('click', () => {
        if (it.simple) {
          ...
          // إضافة البيتزا مباشرة للسلة بكل مكوناتها وصورتها المحددة
          cart.push({
            uid: Date.now(),
            kind: 'pizza',
            name: it.name,
            img: it.img || IMG.pOriginal,
...
const track = $('#menuTrack');
    let sx = 0;
    // التقليب بالصباع يمين وشمال خفيف وسريع (1.5)
    ...
    if (track) {
      gsap.to(track, {
        ...
        scrollTrigger: {
          trigger: '#menu',
          start: 'top top',
          // ⬅️ إرجاع تقل السكرول عند النزول لتحت بزيادة مسافة التثبيت
          end: () => '+=' + Math.max(1800, (track.scrollWidth - window.innerWidth) * 2.2),
          scrub: 0.8, // تقل وثبات عند النزول بالصفحة
          pin: true,
          anticipatePin: 1,
          invalidateOnRefresh: true,
...
/* ================= HERO CINEMATICS (مع إيقاف الفيديو الذكي) ================= */
  ...
  (function(){
    ...
    if(!wrap) return;
    // فك التجميد: لو العجلة معروضة على الشاشة تشتغل فوراً سواء موبايل أو تابلت
    const isWheelActive = () => {
    ...
    function toggle(cat,id){
           if (!mobileSizeChosen && window.innerWidth <= 980) {
        triggerSizeAlert();
        return;
      }
    ...
    wrap.addEventListener('pointerdown',e=>{
      if(!isWheelActive()) return; // فك حظر اللمس على التابلت
...
let prevT = performance.now();
    function loop(t){
      ...
      if(!isWheelActive() || !builderInView) return; // تشغيل دوران البيتزا والعجلة بسلاسة
...
const isPortrait = window.innerHeight > window.innerWidth;
      const isMobilePhone = window.innerWidth <= 600; // هواتف فقط

      // تضييق فائق للهاتف بالطول (0.82) | تابلت بالطول (1.05) | بالعرض واسع (1.25)
      let factor = 1.25;
      if (isPortrait) {
        factor = isMobilePhone ? 0.90 : 1.05;
      }
      const S = hr.width * factor;
    ...
    // إتاحة الدالة للنظام العام ليتم استدعاؤها مع أي أنيميشن
    window.__forno_place_wheel = place;
    gsap.set(wheel, { xPercent: -50, yPercent: -50 });

window.addEventListener('resize', place);
    window.addEventListener('orientationchange', () => {
      setTimeout(place, 100);
      setTimeout(place, 350); // تأكيد التموضع بعد انتهاء حركة تدوير المتصفح
    });
  ...
  /* ================= LOADER / INIT ================= */
  async function boot(){
    console.log('🚀 [FORNO 4/4] boot() fired! Starting progress tween...');

    const loadBar = $('#loadBar');
    const loadPct = $('#loadPct');
    
    // تشغيل عداد التحميل بنعومة من 0 إلى 90%
    const progress = { v: 0 };
    ...
    const jobs = [
      cutout(IMG.doughThin).then(u => { DOUGH_SRC.thin = u; }).catch(() => {}),
      cutout(IMG.doughClassic).then(u => { DOUGH_SRC.classic = u; }).catch(() => {}),
      cutout(IMG.doughThick).then(u => { DOUGH_SRC.thick = u; }).catch(() => {}),
      cutout(IMG.doughCheese).then(u => { DOUGH_SRC.cheese = u; }).catch(() => {}),
    ...
    gsap.to(progress, {
      ...
      onComplete: () => {
        try { localStorage.removeItem('forno_builder_progress'); } catch(e) {}
        ...
        // تفعيل الأزرار الدائرية الأصلية على شاشات الكمبيوتر واللابتوب
        const circleBtns = document.querySelectorAll('.ms-circle-btn');
        circleBtns.forEach(btn => {
          if (btn.dataset.size === (state.size || 'med')) btn.classList.add('active');
          btn.addEventListener('click', () => {
            ...
            // تكبير وتصغير البيتزا بحركة مرنة فخمة على اللابتوب
            if (window.innerWidth > 980) {
              const scaleMap = { small: 0.84, med: 1.0, large: 1.14 };
              gsap.to('#stageHost', { scale: scaleMap[sz] || 1, duration: 0.55, ease: 'back.out(1.5)' });
            }

            // تحديث السعر والفاتورة فوراً
            if (typeof renderSummary === 'function' && curStep === 6) renderSummary();
            toast(`SIZE: ${sz.toUpperCase()} 🍕`);
          });
        });
        // إخفاء شاشة اللودينج بالكامل
        gsap.to('#loader', {
          autoAlpha: 0,
          scale: 1.05,
          duration: 0.6,
  ...
  // 🔥 شرارة الإشعال: مناداة دالة الإقلاع فوراً
  boot();
